...
function addTodo(event) {
  ...
  const arrKiString = JSON.stringify(todos);

  localStorage.setItem("todos", arrKiString);
...
function showTodos() {
  ...
  for (let i = 0; i < todos.length; i++) {
    ul.innerHTML += `<li>${todos[i]} <button onclick="editTodo(${i})">edit</button> <button onclick="deleteTodo(${i})">delete</button></li>`;
  }
}
showTodos();
